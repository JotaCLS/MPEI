...
N = 10000;
X1 = randi(4, [1, N]); % Primeiro numero randi(1 ate 4, numa matriz [1,1000])
X2 = randi(4, [1, N])+4; % Segundo numero randi(1 ate 4, numa matriz [1, 1000] + 4 para ser numeros de 1 a 5)
X = X2-X1; % Variavel aleatoria

labels = 1:7; % so podemos obter 7 valores diferentes de X2-X1 
massa_prob = zeros(1, length(labels)); % funcao massa_prob matriz de zeros 1 a 7, que vai levar com a prob de cada um
for i=labels % de 1 ate 7 
    massa_prob(i) = sum(X == i)/N; % massa prob(i) = nr de vezes que esse numero esta presente na matriz X a dividir pelo total
end

stem(labels, massa_prob) % faz o grafico stem das probabilidades, em que X é os valores e Y as prob
